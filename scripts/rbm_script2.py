...
# 
class RBM(nn.Module):
    def __init__(self, n_vis, n_hid, k, batch_size, anti=False):
        """
...
for h in nhidden:
    ...
    for i in range(epochs):
        length = epochs[i]
        rbm, train, test = create_rbm(train_matrix, test_matrix, h, 2000, length, train_errors=trainerr, test_errors=testerr, rbm=rbm)
        evaluate_rbm(rbm)
        current_epoch += length
        torch.save(rbm.state_dict(), f"./antirbm-{current_epoch}-{h}")
